...
//Limpiar local
var cleanLocal = function (done) {
	// Limpiar folder local
	del.sync(["./**"]);
	done();
}
...
var backup = function (done) {
	var d = new Date();
	src(["./**", "!node_modules/**"])
		.pipe(zip("backup_" + (d.getMonth() + 1) + "-" + d.getDate() + ".zip"))
		.pipe(dest("../../Google Drive/Proyectos/Portafolio/_backup"))
	done();
};

// Checar si nuevo
// exports.checarnuevo = () => (
// 	// Checar si se han cambiado archivos en Google Drive
// 	src(paths.gdpath)

// 		// plugin changed con opciones
// 		.pipe(changed(paths.localpath, {
// 			hasChanged: changed.compareLastModifiedTime
// 		}))
// 		.pipe(dest(paths.localpath))
// );

exports.run_backup = series(backup);
exports.run_upload = series(cleanGD, copyToGD);
// exports.run_download = series(cleanLocal, copyToLocal);